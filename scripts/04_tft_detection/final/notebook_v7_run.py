...
from notebook_v7_detector import StatisticalClassifier7, AdaptiveRollingDetector7
import logging
...
RAW_DATA_PATH = os.environ.get(
    "TFT_RAW_DATA_PATH",
    r"C:\tongji\0 code\00_data\01_EDA",
)
FEATURES_PATH = os.environ.get(
    "TFT_FEATURES_PATH",
    os.path.join(RAW_DATA_PATH, "Time_Series_Features_2024-12-05 00 to 2025-11-27 00_15min.csv"),
)
OFFICIAL_PATH = os.environ.get(
    "TFT_OFFICIAL_PATH",
    os.path.join(RAW_DATA_PATH, "temp_all_official_standart_time_type.csv"),
)

# 直接复制自 Notebook 的真实数据加载与预处理单元。
df_official_real = pd.read_csv(OFFICIAL_PATH)
...
df_features = df_features.drop(
    ['total_medium_post', 'total_medium_comment', 'unique_users_post',
     'unique_users_comment', 'semantic_shift_cross'],
    axis=1,
)

CKPT = os.environ.get("TFT_CHECKPOINT_DIR", "./checkpoints/pretrained_planPC")
CONFIG_PATH = os.environ.get(
    "TFT_CONFIG_PATH",
    os.path.join(os.path.dirname(__file__), "TFT_config.yaml"),
)
tb = TargetBuilder.load(f"{CKPT}/target_builder.pkl")
engine_PC = TFTEngine.load(f"{CKPT}/tft_engine", config_path=CONFIG_PATH)
...
df_result7 = detector7.run(
    ...
    output_path=os.environ.get(
        "TFT_OUTPUT_PATH",
        "./output/adaptive_detection_resultv7.csv",
    )
)
